...
While `moenarchbook` is mainly used as a command line tool, you can also import the
...
## Using `moenarchbook` as a Library

The `moenarchbook` binary is just a wrapper around the `moenarchbook` crate, exposing its
functionality as a command-line program. As such it is quite easy to create your
own programs which use `moenarchbook` internally, adding your own functionality (e.g.
a custom preprocessor) or tweaking the build process.

The easiest way to find out how to use the `moenarchbook` crate is by looking at the
